...
@section('content')
<div class="card">
    <div class="card-header">Edit Kunjungan</div>
    <div class="card-body">
        <form action="{{ route('kunjungan.update', $kunjungan->id_kunjungan) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-3">
                ...
                <select class="form-select @error('id_pasien') is-invalid @enderror" id="id_pasien" name="id_pasien">
                    @foreach($pasien as $p)
                        <option value="{{ $p->id_pasien }}" {{ old('id_pasien', $kunjungan->id_pasien) == $p->id_pasien ? 'selected' : '' }}>{{ $p->nama }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('id_dokter') is-invalid @enderror" id="id_dokter" name="id_dokter">
                    @foreach($dokter as $d)
                        <option value="{{ $d->id_dokter }}" {{ old('id_dokter', $kunjungan->id_dokter) == $d->id_dokter ? 'selected' : '' }}>{{ $d->nama }} ({{ $d->spesialisasi }})</option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <input type="datetime-local" class="form-control @error('tanggal_kunjungan') is-invalid @enderror" id="tanggal_kunjungan" name="tanggal_kunjungan" value="{{ old('tanggal_kunjungan', $kunjungan->tanggal_kunjungan) }}">
                @error('tanggal_kunjungan') <div class="invalid-feedback">{{ $message }}</div> @enderror
            </div>
            <div class="mb-3">
                <label for="diagnosa" class="form-label">Diagnosa</label>
                <textarea class="form-control @error('diagnosa') is-invalid @enderror" id="diagnosa" name="diagnosa">{{ old('diagnosa', $kunjungan->diagnosa) }}</textarea>
